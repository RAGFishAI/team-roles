...
/*Get Id by RouteName*/
func (as ModelStruct) GetModulePermissionByChannelId(id int, DB *gorm.DB, tenantid string) (tblmodper TblModulePermission, err error) {

	if err := DB.Table("tbl_module_permissions").Where("channel_id=? and tenant_id = ?", id, tenantid).First(&tblmodper).Error; err != nil {
...
func (as ModelStruct) DeleteModulePermissioninEntries(id int, DB *gorm.DB, tenantid string) (tblmodper TblModulePermission, err error) {

	if err := DB.Where("channel_id=? and tenant_id = ? ", id, tenantid).Delete(&tblmodper).Error; err != nil {
